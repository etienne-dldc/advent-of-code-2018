(() => {
    ...
    state = '#.##...#.##.##....#.##...#.##.##...#.##.##.##.##.##.##.##.##...#.##...#.##.##.##.##.##.##....#.##...#.##...#.##.##...#.##...#.##.##.##.##.##.##.##'.split('').map(v => v === '#' ? true : false)
    ...
    const hash = state => state.map(x => x ? '#' : '.').join('');

    const target = 50000000000

    const startGen = target - (target % 128);

    console.log(startGen)

    const generations = target;
    let startIndex = startGen - 46 // 83 + 128 * (Math.floor(target / 128) - 1);
    ...
    for (let g = startGen; g < generations; g++) {

        if (g % 128 === 0) {
            console.log(hash(state))
            console.log(g, startIndex);
        }
